def process(character: dict):
    ...
    if "CRIT_rate_0" not in character and "CRIT_rate" in character:
        character["CRIT_rate_0"] = character["CRIT_rate"] - (character["CRIT_rate_1"] if "CRIT_rate_1" in character else 0)
    ...
    GR_CRIT_rate = (10000+(character["CRIT_rate"]+3.11)*character["CRIT_DMG"]) / (10000+character["CRIT_rate"]*character["CRIT_DMG"])*100 - 100
    GR_CRIT_DMG = (10000+character["CRIT_rate"]*(character["CRIT_DMG"]+6.22)) / (10000+character["CRIT_rate"]*character["CRIT_DMG"])*100 - 100
    ...
    print("词条推荐(与2%之间的差距):",round(GR_ATK_1 - 2, 2), round(GR_DMG_bonus - 2, 2), round(GR_CRIT_rate - 2, 2), round(GR_CRIT_DMG - 2,2))
    return final_value, character

def calc_demo(*characters: list[dict]):
    base = characters[0]
    f = 0
